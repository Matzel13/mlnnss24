...
% Zusammenhang zwis-Poolingchen den Pixel verloren.
...
%% Vergleich mit normaler Faltung
...
%% 2D: Matrizen sind Graustufenbilder
...
Yl = extractdata(dlconv(dlarray(X, 'SS'), hLaplace, 0));
montage( {mat2gray(X), mat2gray(Yx), mat2gray(Yy), mat2gray(Yl)}, 'Size', [2, 2]);

%% Groessere Version: LoG
n = 9; 
hLog = fspecial('log', n,3); 
[xg, yg] = meshgrid(1:n); surf(xg,yg,hLog); 
...
%% Ein Farbbild ist eine 'Matrix' mit 3 Indizes
% vornehm ausgedrückt: ein Tensor dritter Stufe
Xc = imread('sherlock.jpg');
