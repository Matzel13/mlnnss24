%% Noch ein Datensatz
load(fullfile('..', 'Daten', 'Data1D2Gr_2.mat')); 
xdim = [-2, 6]; 
...
nexttile(1);
% subplot(4,1,1); 
gscatter(xTrain, zeros(size(xTrain)), trainLbl, 'rb','o'); 
...
nexttile(2);
% subplot(4,1,2); 
gscatter(xTest, zeros(size(xTest)), testLbl, 'rb', 'o'); 
hold off; 
axis([xdim, -0.5, 0.5]); 
set(gca, 'YTick', []), xlabel(''); 
title('Testdaten'); 
...
Pri1 = mean(trainLbl=='1');
Pri2 = mean(trainLbl=='2');
...
[~, trainLblPred] = max(postTrain, [], 2); 
trainLblPred=categorical(trainLblPred);
trainErr2 = mean(trainLblPred ~= trainLbl);
postTest = [cc1(xTest)*Pri1, cc2(xTest)*Pri2];
[~, testLblPred] = max(postTest, [], 2); 
testLblPred=categorical(testLblPred);
testErr2 = mean(testLblPred ~= testLbl);
...
for kk = 1:length(bwVec)
   ...
   [~, lblPred] = max(post, [], 2); 
   lblPred = categorical(lblPred);
   trainErrVec(kk) = mean(lblPred ~= trainLbl);
   % Testfehler
   post = [cc1Win(xTest, bw)*Pri1, cc2Win(xTest, bw)*Pri2];
   [~, lblPred] = max(post, [], 2); 
   lblPred = categorical(lblPred);
   testErrVec(kk) = mean(lblPred ~= testLbl);
end
%%
figure; 
